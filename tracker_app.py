...
from tracker_service import export_analytics_from_config
...
class SettingsDialog(DialogWindow):
    def __init__(self, master: tk.Misc, base_dir: Path, config: dict, on_save):
        ...
        self.analytics_export_var = tk.StringVar(value=deep_get(config, "paths.analytics_export_dir", "analytics_export"))
    ...
    def _build_output_tab(self):
        ...
        _, row = self._add_entry_row(self.output_tab, row, "Analytics export folder", self.analytics_export_var, help_text="Folder for clean CSV files intended for outside analysis.")
    ...
    def _save(self):
        ...
        cfg["paths"]["analytics_export_dir"] = self.analytics_export_var.get().strip() or "analytics_export"
...
class TrackerApp(AppRoot):
    ...
    def export_analytics(self):
        ...
        self._enqueue_log("Analytics export started.")

        def worker():
            try:
                result = export_analytics_from_config(str(self.config_path))
            except Exception as exc:
                self._enqueue_log(f"Analytics export failed: {exc}")
                self._schedule(0, lambda: messagebox.showerror("Export data", str(exc), parent=self))
            else:
                output_dir = Path(str(result["output_dir"]))
                self._enqueue_log(
                    "Analytics export complete: "
                    f"{result['total_posts_exported']} posts, "
                    f"{result['total_snapshots_exported']} snapshots."
                )
                self._schedule(0, self._open_path, output_dir)
            finally:
                self._schedule(0, lambda: self.export_btn.configure(state="normal"))
    ...
    def _ensure_tray(self):
        ...
        try:
            menu = pystray.Menu(
                ...
                pystray.MenuItem("Export data", lambda icon, item: self._schedule(0, self.export_analytics)),
